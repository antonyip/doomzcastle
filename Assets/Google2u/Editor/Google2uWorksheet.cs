...
namespace Google2u
{
    ...
    public class Google2uWorksheet
    {
        ...
        private GUIContent[] _ComboBoxList;
        ...
        public static void UpdateQuery(object in_worksheet)
        {
            ...
            {
                Google2u.PushNotification("Unable to Update " + worksheet.WorksheetName + ". If this problem persists, there is probably something wrong with the spreadsheet.");
            }
        ...
        public static void QueryCells(object in_worksheet)
        {
            ...
            catch (Exception ex)
            {
                ...
                Google2u.PushNotification("Unable to Query " + worksheet.WorksheetName + ". If this problem persists, there is probably something wrong with the spreadsheet." + ex.Message);
            }
        ...
        public void DrawGUIFull(EditorGUILayoutEx in_layout)
        {
            ...
            if (WorksheetQueryStatus != QueryStatus.QueryComplete && WorksheetQueryStatus != QueryStatus.Idle && RowsDisplay.Length == 0)
            {
                EditorGUILayout.LabelField(Google2u.LocalizationInfo.Localize(Localization.rowIds.ID_MESSAGE_QUERYING_CELLS) + Google2u.Ellipses);
            }
            else
            if (RowsDisplay.Length == 0)
            {
                if(WorksheetQueryStatus == QueryStatus.QueryComplete || WorksheetQueryStatus == QueryStatus.Idle)
                    EditorGUILayout.LabelField(Google2u.LocalizationInfo.Localize(Localization.rowIds.ID_MESSAGE_EMPTY_WORKSHEET));
            }
            else
            {
                ...
                for (var i = 0; i < RowsDisplay.Length; i++)
                {
                    ...
                    if (i == 1 && UseTypeRow)
                    {
                        for (var j = 0; j < RowsDisplay[i].Count; j++)
                        {
                            ...
                            {
                                var newType = (SupportedType)Enum.Parse(typeof(SupportedType), _ComboBoxList[newSelected].text, true);
        ...
        private Dictionary<int, Google2uGUIUtil.ComboBox> _ComboBoxes = new Dictionary<int, Google2uGUIUtil.ComboBox>(); 
        ...
        public void DoExport()
        {
            ...
            _StaticDbPath = Google2uGUIUtil.GetString("g2uStaticDBResourcesDirectory", Google2u.Google2uGenPath("STATICDB"));
