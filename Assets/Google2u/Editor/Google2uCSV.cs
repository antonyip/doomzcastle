...
namespace Google2u
{
    public partial class Google2u : EditorWindow
    {
        ...
        {
            var doAutoQuote = in_string.Contains(",") || in_string.Contains("\"") || in_string.Contains(Environment.NewLine) || in_string.Contains("\n");
        ...
        public static string ExportCsvString(Google2uWorksheet in_sheet, Google2uExportOptions in_options)
        {
            ...
            foreach (var row in in_sheet.Rows)
            {
                ...
                for (int i = 0; i < in_sheet.Rows[0].Count; i++)
                {
                    ...
                    var tmpRet = in_options.EscapeCSVStrings ? SanitizeDf(row[i].CellValueString) : AutoQuote(row[i].CellValueString);
