    ...
    public class Google2uManualWorkbook : WorkbookBase
    {
        ...
        public override void Update()
        {
            if (WorksheetQueryStatus == QueryStatus.QueryComplete)
            {
                ...
                {
                    if (ActiveWorksheetIndex > -1 && Worksheets[ActiveWorksheetIndex] != null && Google2uWorksheet == Worksheets[ActiveWorksheetIndex])
            ...
            switch (WorksheetQueryStatus)
            {
                ...
                case QueryStatus.QueryComplete:
                    if (WorksheetsDisplay.Length > 0)
                    {
                        ...
                        ActiveWorksheetIndex = EditorGUILayout.Popup(Google2u.LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_ACTIVE_WORKSHEET) + ": ", ActiveWorksheetIndex,
                            worksheetNames);
                        ...
                        var newExportType = (ExportType)EditorGUILayout.EnumPopup(Google2u.LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_EXPORT_AS) + ": ", activeWorksheet.WorksheetExportType, GUILayout.Width(300));
        ...
        {
            var ret = true;
            ShowSpreadsheet = Google2uGUIUtil.GetBool("mworkbook" + WorkbookName.Replace(' ', '_') + "_Open", ShowSpreadsheet);
            var mainFadeArea = in_layout.BeginFadeArea(ShowSpreadsheet, WorkbookName, "mworkbook" + WorkbookName.Replace(' ', '_'), in_layout.OuterBox, in_layout.OuterBoxHeader);
                ...
                switch (WorksheetQueryStatus)
                {
                    ...
                    case QueryStatus.QueryComplete:
                        if (Worksheets.Count > 0)
                        {
                            ...
                            content = new GUIContent(in_layout.EditButton, Google2u.LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_VIEW_WORKBOOK));
                            if (GUILayout.Button(content, GUILayout.Height(EditorGUILayoutEx.ButtonHeight), GUILayout.Width(EditorGUILayoutEx.ButtonWidth)))
                            {
                            ...
                            content = new GUIContent(in_layout.DeleteButton, Google2u.LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_REMOVE_WORKBOOK));
                            if (GUILayout.Button(content, GUILayout.Height(EditorGUILayoutEx.ButtonHeight), GUILayout.Width(EditorGUILayoutEx.ButtonWidth)))
                            {
                                if (EditorUtility.DisplayDialog(Google2u.LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_REMOVE_WORKBOOK),
                                    Google2u.LocalizationInfo.Localize(Localization.rowIds.ID_MESSAGE_REMOVE_WORKBOOK), 
