...
namespace Google2u
{
    public partial class Google2u : EditorWindow
    {
        ...
        private static OAuth2Parameters _authParameters = null;
        ...
        public bool ShowSettingsLanguage = false;
        public bool ShowSettingsPaths = false;
        public bool ShowSettingsPathGameObjectExport = false;
        public bool ShowSettingsPathStaticExport = false;
        public bool ShowSettingsPathXMLExport = false;
        public bool ShowSettingsPathJSONExport = false;
        public bool ShowSettingsPathCSVExport = false;
        public bool ShowSettingsPathNGUIExport = false;
        public bool ShowSettingsPathPlaymakerExport = false;
        public bool ShowSettingsPrefs = false;

        // Workbooks Menu
        public bool ShowWorkbooks = false;
        public bool ShowWorkbooksAccount = false;
        public bool ShowWorkbooksManual = false;
        public bool ShowWorkbooksUpload = false;
        public bool ShowHiddenWorkbooks = false;

        // Tools Menu
        public bool ShowTools = false;
        public bool ShowToolsOptions = false;
        public bool ShowToolsWorksheets = false;

        // Help Menu
        public bool ShowHelp = false;
        public bool ShowHelpContact = false;
        public bool ShowHelpDocs = false;
        public bool ShowHelpLog = false;
        ...
        void DrawLoginGUI()
        {
            ...
            var loginFadeArea = InstanceData.Service == null
                ...
                : MyGUILayout.BeginFadeArea(ShowLogin,
                    LocalizationInfo.Localize(Localization.rowIds.ID_MESSAGE_LOGGED_IN_AS), "login", MyGUILayout.OuterBox, MyGUILayout.OuterBoxHeader);
            ...
            if (loginFadeArea.Show())
            {
                ...
                {
                    EditorGUILayout.LabelField(LocalizationInfo.Localize(Localization.rowIds.ID_MESSAGE_PROCESSING_LOGIN) + Google2u.Ellipses);
                }
                else if (InstanceData.Commands.Contains(GFCommand.WaitForRetrievingWorkbooks))
                {
                    EditorGUILayout.LabelField(LocalizationInfo.Localize(Localization.rowIds.ID_MESSAGE_RETRIEVING_WORKBOOKS) + Google2u.Ellipses);
                }
                else
                {
        ...
        {
            var settingsFadeArea = MyGUILayout.BeginFadeArea(ShowSettings, LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_SETTINGS), "settings", MyGUILayout.OuterBox, MyGUILayout.OuterBoxHeader);
            ShowSettings = settingsFadeArea.Open;
            Google2uGUIUtil.SetBool("ShowSettings", ShowSettings);
            if (settingsFadeArea.Show())
            {

                var showSettingsLanguage = MyGUILayout.BeginFadeArea(ShowSettingsLanguage, LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_LANGUAGE), "settingsLanguage", MyGUILayout.InnerBox, MyGUILayout.InnerBoxHeader);
                ...
                if (showSettingsLanguage.Show())
                {
                    ...
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.LabelField(LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_EDITOR_LANGUAGE) + ": ", GUILayout.Width(100));
                    

                    GUI.SetNextControlName("Clear");
                    LocalizationInfo.LanguagesIndex = EditorGUILayout.Popup(LocalizationInfo.LanguagesIndex, LocalizationInfo.LanguageOptions);
                ...
                var showSettingsPaths = MyGUILayout.BeginFadeArea(ShowSettingsPaths, LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_EXPORTERS), "settingsPaths", MyGUILayout.InnerBox, MyGUILayout.InnerBoxHeader);
                ...
                if (showSettingsPaths.Show())
                {
                    ...
                    if (GUILayout.Button(LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_GENERATE_PATHS), EditorStyles.toolbarButton))
                    {
                    ...
                    var showSettingsPathGameObjectExport = MyGUILayout.BeginFadeArea(ShowSettingsPathGameObjectExport, 
                        ...
                        LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_EXPORT), "showSettingsPathGameObjectExport", MyGUILayout.OuterBox, MyGUILayout.OuterBoxHeader);
                    ...
                    if (showSettingsPathGameObjectExport.Show())
                    {
                        ...
                        EditorGUILayout.LabelField(LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_OBJECT_DATABASE) + " " +
                        ...
                        var content = new GUIContent(MyGUILayout.BrowseButton, LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_CHOOSE_FOLDER));
                        ...
                        content = new GUIContent(MyGUILayout.BrowseButton, LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_CHOOSE_FOLDER));
                        if (GUILayout.Button(content, GUILayout.Height(EditorGUILayoutEx.ButtonHeight), GUILayout.Width(EditorGUILayoutEx.ButtonWidth)))
                        {
                    ...
                    var showSettingsPathStaticExport = MyGUILayout.BeginFadeArea(ShowSettingsPathStaticExport, LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_STATIC_DATABASE) + " " + LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_EXPORT), "showSettingsPathStaticExport", MyGUILayout.OuterBox, MyGUILayout.OuterBoxHeader);
                    ...
                    if (showSettingsPathStaticExport.Show())
                    {
                        ...
                        EditorGUILayout.LabelField(LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_STATIC_DATABASE) + " " + LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_RESOURCES_DIR) + ": ");
                        EditorGUILayout.BeginHorizontal();
                    ...
                    var showSettingsPathXMLExport = MyGUILayout.BeginFadeArea(ShowSettingsPathXMLExport, "XML " + LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_EXPORT), "showSettingsPathXMLExport", MyGUILayout.OuterBox, MyGUILayout.OuterBoxHeader);
                    ...
                    if (showSettingsPathXMLExport.Show())
                    {
                        ...
                        EditorGUILayout.LabelField("XML " + LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_EXPORT_DIR) + ": ");
                        EditorGUILayout.BeginHorizontal();
                        ...
                        if (GUILayout.Button(content, GUILayout.Height(EditorGUILayoutEx.ButtonHeight), GUILayout.Width(EditorGUILayoutEx.ButtonWidth)))
                        {
                            ...
                            if ((xmlDirectory.Length == 0) ||
                                    (xmlDirectory.IndexOf(InstanceData.ProjectPath, StringComparison.InvariantCultureIgnoreCase) != 0))
                            {
                                InstanceData.Messages.Add(new G2GUIMessage(
                                    GFGUIMessageType.InvalidEditorDirectory,
                    ...
                    var showSettingsPathJSONExport = MyGUILayout.BeginFadeArea(ShowSettingsPathJSONExport, "JSON " + LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_EXPORT), "showSettingsPathJSONExport", MyGUILayout.OuterBox, MyGUILayout.OuterBoxHeader);
                    ...
                    if (showSettingsPathJSONExport.Show())
                    {
                        ...
                        EditorGUILayout.LabelField("JSON " + LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_EXPORT_DIR) + ": ");
                        EditorGUILayout.BeginHorizontal();
                        ...
                        if (GUILayout.Button(content, GUILayout.Height(EditorGUILayoutEx.ButtonHeight), GUILayout.Width(EditorGUILayoutEx.ButtonWidth)))
                        {
                            ...
                            if ((jsonDirectory.Length == 0) ||
                                    (jsonDirectory.IndexOf(InstanceData.ProjectPath, StringComparison.InvariantCultureIgnoreCase) != 0))
                            {
                                InstanceData.Messages.Add(new G2GUIMessage(
                                    GFGUIMessageType.InvalidEditorDirectory,
                                    LocalizationInfo.Localize(Localization.rowIds.ID_ERROR_INVALID_DIR)));
                    ...
                    var showSettingsPathCSVExport = MyGUILayout.BeginFadeArea(ShowSettingsPathCSVExport, "CSV " + LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_EXPORT), "showSettingsPathCSVExport", MyGUILayout.OuterBox, MyGUILayout.OuterBoxHeader);
                    ...
                    if (showSettingsPathCSVExport.Show())
                    {
                        ...
                        EditorGUILayout.LabelField("CSV " + LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_EXPORT_DIR) + ": ");
                        EditorGUILayout.BeginHorizontal();
                        ...
                        if (GUILayout.Button(content, GUILayout.Height(EditorGUILayoutEx.ButtonHeight), GUILayout.Width(EditorGUILayoutEx.ButtonWidth)))
                        {
                            ...
                            if ((csvDirectory.Length == 0) ||
                                    (csvDirectory.IndexOf(InstanceData.ProjectPath, StringComparison.InvariantCultureIgnoreCase) != 0))
                            {
                                InstanceData.Messages.Add(new G2GUIMessage(
                                    GFGUIMessageType.InvalidEditorDirectory,
                                    LocalizationInfo.Localize(Localization.rowIds.ID_ERROR_INVALID_DIR)));
                    ...
                    if (FoundNgui)
                    {
                        var showSettingsPathNGUIExport = MyGUILayout.BeginFadeArea(ShowSettingsPathNGUIExport,
                            "NGUI " + LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_EXPORT), "showSettingsPathNGUIExport", MyGUILayout.OuterBox,
                        ...
                        if (showSettingsPathNGUIExport.Show())
                        {
                            ...
                            EditorGUILayout.LabelField("NGUI " + LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_EXPORT_DIR) + ": ");
                            EditorGUILayout.BeginHorizontal();
                            ...
                            if (GUILayout.Button(content, GUILayout.Height(EditorGUILayoutEx.ButtonHeight), GUILayout.Width(EditorGUILayoutEx.ButtonWidth)))
                            {
                                ...
                                if ((nguiDirectory.Length == 0) ||
                                    (nguiDirectory.IndexOf(InstanceData.ProjectPath, StringComparison.InvariantCultureIgnoreCase) != 0))
                                {
                                    InstanceData.Messages.Add(new G2GUIMessage(
                                        GFGUIMessageType.InvalidEditorDirectory,
                                    LocalizationInfo.Localize(Localization.rowIds.ID_ERROR_INVALID_DIR)));
                    ...
                    if (FoundPlaymaker)
                    {
                        ...
                        if (showSettingsPathPlaymakerExport.Show())
                        {
                            ...
                            EditorGUILayout.LabelField("Playmaker " + LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_EXPORT_DIR) + ": ");
                            EditorGUILayout.BeginHorizontal();
                            ...
                            if (GUILayout.Button(content, GUILayout.Height(EditorGUILayoutEx.ButtonHeight), GUILayout.Width(EditorGUILayoutEx.ButtonWidth)))
                            {
                                ...
                                if ((playmakerDirectory.Length == 0) ||
                                    (playmakerDirectory.IndexOf(InstanceData.ProjectPath, StringComparison.InvariantCultureIgnoreCase) != 0))
                                {
                                    InstanceData.Messages.Add(new G2GUIMessage(
                                        GFGUIMessageType.InvalidEditorDirectory,
                                    LocalizationInfo.Localize(Localization.rowIds.ID_ERROR_INVALID_DIR)));
                ...
                var showSettingsPrefs = MyGUILayout.BeginFadeArea(ShowSettingsPrefs, "Preferences", "settingsPrefs", MyGUILayout.InnerBox, MyGUILayout.InnerBoxHeader);
        ...
        void DrawWorkbooksGUI()
        {
            // WORKBOOKS
            var workbooksFadeArea = MyGUILayout.BeginFadeArea(ShowWorkbooks, LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_WORKBOOKS), "workbooks", MyGUILayout.OuterBox, MyGUILayout.OuterBoxHeader);
            ...
            if (workbooksFadeArea.Show())
            {
                ...
                {
                    // Account
                    var showWorkbooksAccount = MyGUILayout.BeginFadeArea(ShowWorkbooksAccount, LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_ACCOUNT_WORKBOOKS),
                    ...
                    if (showWorkbooksAccount.Show())
                    {
                        ...
                        {
                            if (GUILayout.Button(LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_RELOAD_WORKBOOKS)))
                            {
                                InstanceData.Commands.Add(GFCommand.RetrieveWorkbooks);
                            }
                            else
                            {
                                // TODO account workbook options
                                ShowHiddenWorkbooks = Google2uGUIUtil.GetBool("ShowHiddenWorkbooks", ShowHiddenWorkbooks);
                ...
                // Manual
                var showWorkbooksManual = MyGUILayout.BeginFadeArea(ShowWorkbooksManual, LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_MANUAL_WORKBOOKS), "workbooksManual", MyGUILayout.InnerBox, MyGUILayout.InnerBoxHeader);
                ...
                if (showWorkbooksManual.Show())
                {
                    ...
                    {
                        var deleteWorkbooks = InstanceData.ManualWorkbooksDisplay.Where(in_Google2uSpreadsheet => in_Google2uSpreadsheet.DrawGUIList(MyGUILayout, false) == false).ToList();
                        ...
                        if (refreshManualWorkbooks)
                        {
                            ...
                            Google2uGUIUtil.SetString(InstanceData.ProjectPath + "_ManualWorkbookCache", InstanceData.ManualWorkbookCache);
                        }
                        EditorGUILayout.BeginHorizontal();
                        ...
                        var content = new GUIContent(MyGUILayout.AddButton, LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_ADD_WORKBOOK));
                        if (GUILayout.Button(content, GUILayout.Height(EditorGUILayoutEx.ButtonHeight), GUILayout.Width(EditorGUILayoutEx.ButtonWidth)))
                        {
                ...
                {
                    // Upload
                    var showWorkbooksUpload = MyGUILayout.BeginFadeArea(ShowWorkbooksUpload, LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_UPLOAD_WORKBOOK),
                    ...
                    if (showWorkbooksUpload.Show())
                    {
                        ...
                        {
                            EditorGUILayout.LabelField(LocalizationInfo.Localize(Localization.rowIds.ID_MESSAGE_UPLOADING_WORKBOOK) + ": " +
                                InstanceData.WorkbookUploadProgress + "% " + LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_COMPLETE) + Google2u.Ellipses);
                        }
                        else
                        {
                            EditorGUILayout.BeginHorizontal();
                            var content = new GUIContent(MyGUILayout.BrowseButton, LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_CHOOSE_FILE));
                            ...
                            if (GUILayout.Button(content, EditorStyles.miniButton, GUILayout.Width(24)))
                            {
                                var workbookpath = EditorUtility.OpenFilePanel(
                                        LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_SELECT_WORKBOOK_PATH), EditorApplication.applicationPath, "*.xls;*.xlsx;*.ods;*.csv;*.txt;*.tsv");
                            ...
                            content = new GUIContent(MyGUILayout.UploadButton, LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_UPLOAD_WORKBOOK));
                            if (GUILayout.Button(content, GUILayout.Height(EditorGUILayoutEx.ButtonHeight), GUILayout.Width(EditorGUILayoutEx.ButtonWidth)))
                            {
                                InstanceData.Messages.Clear();
                                if (IsValidWorkbookPath(InstanceData.WorkbookUploadPath))
                                    ...
                                    InstanceData.Messages.Add(new G2GUIMessage(GFGUIMessageType.InvalidUploadDirectory, LocalizationInfo.Localize(Localization.rowIds.ID_ERROR_INVALID_UPLOAD_DIR)));
                            }
                            EditorGUILayout.EndHorizontal();
                        }
                    }
        ...
        static bool IsValidWorkbookPath(string in_path)
        {
        ...
        void DrawHelpGUI()
        {
            var helpFadeArea = MyGUILayout.BeginFadeArea(ShowHelp, LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_HELP), "Help", MyGUILayout.OuterBox, MyGUILayout.OuterBoxHeader);
            ShowHelp = helpFadeArea.Open;
            Google2uGUIUtil.SetBool("ShowHelp", ShowHelp);
            if (helpFadeArea.Show())
            {
                var showHelpContact = MyGUILayout.BeginFadeArea(ShowHelpContact, LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_CONTACT), "HelpContact", MyGUILayout.InnerBox, MyGUILayout.InnerBoxHeader);
                ShowHelpContact = showHelpContact.Open;
                Google2uGUIUtil.SetBool("ShowHelpContact", ShowHelpContact);
                if (showHelpContact.Show())
                {
                    EditorGUILayout.BeginHorizontal();
                    var content = new GUIContent(MyGUILayout.LitteratusLogo, LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_BROWSE_LITTERATUS));
                    ...
                    EditorGUILayout.BeginHorizontal();
                    content = new GUIContent(MyGUILayout.UnityLogo, LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_BROWSE_UNITY));
                    if (GUILayout.Button(content))
                    {
                        Application.OpenURL("http://www.unity3d.com");
                    }
                    EditorGUILayout.EndHorizontal();
                    EditorGUILayout.LabelField(LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_CREATED_WITH_UNITY) + " \u00a9 " +
                        LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_COPYRIGHT_UNITY));
                }
                MyGUILayout.EndFadeArea();

                var showHelpDocs = MyGUILayout.BeginFadeArea(ShowHelpDocs, LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_DOCUMENTATION), "HelpDocs", MyGUILayout.InnerBox, MyGUILayout.InnerBoxHeader);
                ...
                if (showHelpDocs.Show())
                {
                    ...
                    ShowDocsAtStartup = EditorGUILayoutEx.ToggleInput(LocalizationInfo.Localize(Localization.rowIds.ID_LABEL_SHOWONSTARTUP), ShowDocsAtStartup, "ShowDocsAtStartup");
                }
                MyGUILayout.EndFadeArea();
        ...
        void OnGUI()
        {
            ...
            if (!stylesLoaded)
            {
                if (!LoadStyles())
                {
                    GUILayout.Label("The folder " + EditorAssets + "/ was not found or some custom styles in it do not exist.");
                    return;
                }
            }
            ...
            {
                EditorGUILayout.HelpBox(LocalizationInfo.Localize(Localization.rowIds.ID_ERROR_BUILD_TARGET), MessageType.Error);
                return;
            }
            ...
            {
                EditorGUILayout.HelpBox("Google2u is an Editor-Only application. Functionality is unavailable in Play Mode.", MessageType.Info);
                return;
            }
        ...
        public bool LoadStyles()
        {
            ...
            if (!File.Exists(projectPath + EditorAssets + "/Google2uAssets"))
            {
                ...
                while (dirQueue.Count > 0)
                {
                    ...
                    if (File.Exists(dir.FullName + "/Google2uAssets"))
                    {
            ...
            if (UseDarkSkin)
            {
                Google2uSkin = AssetDatabase.LoadAssetAtPath(EditorAssets + "/DarkSkin/Google2uDark.guiskin", typeof(GUISkin)) as GUISkin;
            }
            else
            {
                Google2uSkin = AssetDatabase.LoadAssetAtPath(EditorAssets + "/LightSkin/Google2uLight.guiskin", typeof(GUISkin)) as GUISkin;
            }
